...
module.exports = grammar({
  ...
  conflicts: $ => [
    ...
    [$.preproc_call_expression2, $._type_specifier, $._declarator, $._expression],
  ],
  ...
  rules: {
    ...
    _top_level_item: $ => choice(
      ...
      $.preproc_call,
      alias($.preproc_call_expression2, $.call_expression), // useful for prepro calls without semicolon
      // alias(prec(PREC.DEFAULT, seq(
      //   field('function', $.identifier),
      //   field('arguments', alias($.preproc_argument_list, $.argument_list))
      // )), $.call_expression),
    ),
    preproc_call_expression2: $ => prec(PREC.DEFAULT, seq(
      field('function', $.identifier),
      field('arguments', alias($.preproc_argument_list, $.argument_list))
    )),
    ...
    inline_asm_expression: $ => prec(PREC.CALL, seq(
      'asm',
      '(',
      // field('asm_template', repeat1($.string_literal)),
      repeat1($.string_literal),
      repeat1(seq( ':', commaSep($.inline_asm_operand))),
      // field('operands', repeat1(seq(':', //commaSep(
      //   seq($.string_literal)
      //   // seq(optional(seq('[', $.identifier, ']')), $.string_literal, optional(seq('(', $.identifier, ')')))
      // //)
      // ))),
      ')'
    )),

    inline_asm_operand: $ => seq( 
